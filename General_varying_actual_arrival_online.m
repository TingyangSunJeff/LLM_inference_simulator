...
for i = 1:num_N
  ...
  for mc = 1:num_MC_runs
    ...
    %% — Fix |R| once from λ_base = 0.5 —
    lambda_base = 0.0005;  % requests/s
    [time_per_request_base] = BPRR_heuristic_general_extended(1,128,initial_delay,RTT,RTT_input,...
                                                              tau,tau_in,L,sm,sc,M);
    mean_req_base = lambda_base*time_per_request_base;
    std_req_base  = sqrt(lambda_base*time_per_request_base);
    R_max    = floor((sum(M)-sm*(L+n_server)) / (sc*(L+n_server)));
    R  = min(round(mean_req_base + std_req_base), R_max);
    fprintf('Fixed target load |R| = %d (from λ=%.3g)\n', R, lambda_base);
    %% Proposed
    t0 = tic;
...
ax.FontSize = 24;
% ax.YLim = [0 35];
xlabel('Actual rate (requests/s)','Interpreter','latex','FontSize',24,'FontWeight','bold');
...
exportgraphics(gcf,sprintf('Inference_vs_actual_lambda_%s.pdf',network_name),'ContentType','vector');
...
exportgraphics(gcf,sprintf('Runtime_vs_actual_lambda_%s.pdf',network_name),'ContentType','vector');
