...
n_request_client = 8; % #requests per client; must have #requests <= sum(max_sessions)/ceil(L/m) (to be feasible)
...
remote_rtt_max = 200; 
rttfile = ['data/rtt_special_' num2str(n_local_server) 'localserver_' num2str(n_remote_server) 'remoteserver.mat']; % file storing the pre-generated RTTs (for fair comparison across different workloads specified by n_request_client, lc)
...
methods = {'Petals','RR only','GSA-BPRR'};
cmap = {'k','m','r'};
fsize = 16;
