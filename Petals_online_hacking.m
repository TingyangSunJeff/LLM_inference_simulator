function [soln_f_petals,soln_a_petals,soln_m_petals,soln_val_petals,ttft,Waiting_time,max_concurrent] = Petals_online_hacking(server_types, soln_m_heu, num_key_value_groups, order,throughput,RTT_raw,overhead_delay_petals,alloc_delay,d_model, inter_request_time,lc,initial_delay,RTT,RTT_input,tau,tau_input,L,sm,sc,M,E,link, allocation_delay)
...
session_capacity = floor((M - soln_m_petals*sm) ./ (soln_m_petals*sc)); % session_capacity(j): max #inference sessions server j can host (assuming each token needs to go through 
