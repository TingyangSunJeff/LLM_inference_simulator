...
for idx = 1:num_C
    C = C_values(idx);
    lambda = 5e-4 * (C/9);
    n_requests  = round(100 * (C/9));
    
    fprintf('\n===== Sweeping C = %d lambda = %f =====\n', C, lambda);
    ...
    Time_proposed_runtime               = zeros(1,num_MC_runs);
    Time_petals_runtime                 = zeros(1,num_MC_runs);
    ...
    for mc_run = 1:num_MC_runs
        ...
        Time_proposed_runtime(mc_run)          = toc(t0);
        ...
        Time_petals_runtime(mc_run)            = toc(t1);
        ...
        [~,~,~,soln_val_po, ttft_po, wait_po, ~] = ...
            Petals_online(server_types, cache_bytes_per_block, num_key_value_groups, ...
                          ...
                          d_model, inter_request_time, lc, initial_delay, RTT, RTT_input, ...
                          tau, tau_input, L, sm, sc, M, E, link, allocation_delay_petals);
        Time_petals_optimized_order_runtime(mc_run)   = toc(t2);
        ...
        [~, soln_val_prr, ttft_prr, wait_prr] = ...
            Petals_optimized_routing(cache_bytes_per_block, soln_a, soln_m, ...
                                     ...
                                     tau, tau_input, L, sm, sc, M, E, link, allocation_delay_petals);
        Time_petals_optimized_rr_runtime(mc_run)       = toc(t3);
        ...
        [~,~,~,soln_val_pnum, ttft_pnum, wait_pnum, ~] = ...
            Petals_online_hacking(server_types, soln_m_heu, num_key_value_groups, ...
                                  ...
                                  d_model, inter_request_time, lc, initial_delay, RTT, RTT_input, ...
                                  tau, tau_input, L, sm, sc, M, E, link, allocation_delay_petals);
        Time_petals_optimized_number_runtime(mc_run)    = toc(t4);
    ...
    avg_runtime_prop(idx)      = mean(Time_proposed_runtime);
    avg_runtime_petals(idx)    = mean(Time_petals_runtime);
    avg_runtime_opt_order(idx) = mean(Time_petals_optimized_order_runtime);
    avg_runtime_opt_rr(idx)    = mean(Time_petals_optimized_rr_runtime);
    avg_runtime_opt_num(idx)   = mean(Time_petals_optimized_number_runtime);
end
...
filename = ['Inference_vs_C_lam_' network_name '.pdf'];
exportgraphics(gcf, filename, ...
               'ContentType','vector', ...
               'BackgroundColor','none');

%% — Plot average runtime for all 5 methods —
figure;
set(gcf,'DefaultAxesFontName','Times New Roman','DefaultTextFontName','Times New Roman');

x = 1:num_C;  % equally spaced positions
runtime_data = [ ...
    avg_runtime_prop;
    avg_runtime_petals;
    ...
    avg_runtime_opt_rr;
    avg_runtime_opt_num ];
runtime_reordered = runtime_data(methodOrder, :)';  % same reorder you used
...
outfn = sprintf('Runtime_vs_C_lam_%s.pdf', network_name);
exportgraphics(gcf, outfn, 'ContentType','vector', 'BackgroundColor','none');
